...
After separating the data by race, for ELA testing in @fig-ela-race, we can see that primarily Asian and White schools meet benchmarks proportionally higher across time as compared to primarily Black and Hispanic schools. Additionally, primarily Asian schools were the only set of schools that saw consistent improvement in meeting ELA scores over time. The reason for this is unclear, and perhaps with more primarily Asian schools, the trend would change. For math, in @fig-math-race, we can see the same pattern of primarily Asian and White schools meeting standards more than primarily Black and Hispanic schools. We can also see a similar trend of minor improvement two years after the pandemic for both ELA and math. Here, we can see the predicted 'gap' appearing within the data, with Black and Hispanic students meeting benchmarks less than their Asian and White peers.
...
@fig-title-one provides the primary racial breakdown of the Title I schools. It should be noted that each school is represented at most four times in the dataset, contributing to the high count for each bar. For more on this, see the *Appendix: Technical Information*.
...
*Grants: Title I, Part A -- Improving Basic Programs*. Illinois State Board of Education. (n.d.). <https://www.isbe.net/Pages/Title-I-Part-A.aspx>

U.S. Census Bureau. (n.d.). *Census: Race*. Census: Quick Facts. <https://www.census.gov/quickfacts/fact/note/US/RHI625222>
...
It was mentioned in the data overview that there are variables for the racial groupings of Black, Asian, Hispanic, Multiracial, Native American/Alaskan Native, Pacific Islander/Hawaiian, White, and Not Available (Census: Race). However, in this report, only Asian, Black, Hispanic, and White are examined. This is due to the structure of the analysis examining the primary racial group at each school, and no schools had primarily racial groups of the other categorizations. I chose not to add the Hawaiian and Pacific Islander categories to the 'Asian' group due to discourse around whether or not to add all those groups to the umbrella of 'Asian', and whether that causes erasure of the differing Hawaiian and Pacific Islander identities.
...
In the variable that determines the level of school, there are high schools, middle schools, and elementary schools. While this data only looks at scores from 3rd to 8th grade, some high schools serve grades 7th to 12th. Therefore, that is why there are high schools included in the dataset. For further information on the levels of schools in the dataset, see *Appendix: Extra Explorations*.
